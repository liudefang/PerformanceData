...
public class FpsTimerTask extends TimerTask {
	...
	private PerfPara fpsPara;

	public FpsTimerTask(){
		fpsPara = PerfManager.getParaLists().get("FPS");
	}
	/*
	...
	public void run() {
		/*
		 * Timer的启动无法避免，所以这里加root保护 若未root，直接返回，减少空消耗
		 */
		if (!FrameUtils.isHasSu()) {
			return;
		}
		...
		if (realCostTime > 0.0F) {
			...
			if(fpsPara == null)
			{
				LogUtils.log("fpsPara is null stop the FpsTimeTask!!");
				stopCurrentTask();
				return;
			}
			PerfManager.getInstance().addHistory(fpsPara,String.valueOf(fpsResult),fpsResult);
			LogUtils.log("FPS:" + fpsResult);
		}
	...
	public static synchronized int getFrameNum() throws IOException {
		...
		int frameNum;
		LogUtils.log("frameNumString："+frameNumString);
